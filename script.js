...
function speakText(text) {
  if ('speechSynthesis' in window) {
    ...
    const onVoicesChanged = () => {
      ...
      console.log(`Платформа: ${isIOS ? 'iOS' : isAndroid ? 'Android' : isMobile ? 'Mobile' : 'Desktop'}`);

      // Приоритетные списки голосов для мужской и женской версий
      const maleVoicePriority = [
        // Android голоса (Google TTS)
        "Google UK English Male",
        "Google британский английский (мужской)",
        "en-gb-x-gbb-local", // Android системный голос
        "en-gb-x-gbd-local",
        "English (United Kingdom)",
        "English United Kingdom",
        
        // Apple голоса (Safari, iOS, macOS)
      ...
      const femaleVoicePriority = [
        ...
        "en-gb-x-gbc-local", // Android системный голос
        "en-gb-x-gba-local",
        "English (United Kingdom)",
        "English United Kingdom",
      ...
      const findFallbackVoice = (isFemale = false) => {
        ...
        if (isAndroid) {
          console.log("Android обнаружена - используем специальную логику выбора голосов");
          console.log("Доступные английские голоса на Android:", englishVoices.map(v => v.name));
          
          // На Android часто есть только один голос, попробуем его настроить через параметры
          if (englishVoices.length === 1) {
            console.log("Найден только один голос на Android, будем использовать его с разными параметрами");
            return englishVoices[0];
          }
          
          // Если голосов больше одного, ищем по полу
          const androidGenderKeywords = {
            male: ['male', 'man', 'google uk english male', 'en-gb-x-gbb', 'en-gb-x-gbd'],
            female: ['female', 'woman', 'google uk english female', 'en-gb-x-gbc', 'en-gb-x-gba']
          };

          const targetKeywords = isFemale ? androidGenderKeywords.female : androidGenderKeywords.male;
          
          for (const voice of englishVoices) {
            const voiceName = voice.name.toLowerCase();
            const voiceLang = voice.lang.toLowerCase();
            if (targetKeywords.some(keyword => 
                voiceName.includes(keyword) || voiceLang.includes(keyword)
            )) {
              console.log(`Найден голос для Android: ${voice.name}`);
              return voice;
            }
          }
          
          // Если не нашли по полу, используем первый доступный
          return englishVoices[0];
        }
      ...
      if (isAndroid) {
        ...
        utterance.rate = 0.8; // Медленнее для Android
        // На Android используем разные параметры для имитации разных голосов
        if (version === 'girl') {
          utterance.pitch = 1.5; // Высокий тон для женского голоса
          utterance.rate = 0.9;
        } else {
          utterance.pitch = 0.7; // Низкий тон для мужского голоса
          utterance.rate = 0.8;
        }
        utterance.volume = 1.0;
      } else if (isIOS) {
        utterance.lang = 'en-US'; // iOS лучше работает с en-US чем с en-GB
        utterance.rate = 0.9; // Немного медленнее для iOS
...
// Функция для отладки голосов на Android
function debugAndroidVoices() {
  if (/Android/.test(navigator.userAgent)) {
    const voices = window.speechSynthesis.getVoices();
    console.log("=== Отладка голосов Android ===");
    console.log(`Всего голосов: ${voices.length}`);
    
    voices.forEach((voice, index) => {
      console.log(`${index}: "${voice.name}"`);
      console.log(`   Язык: ${voice.lang}`);
      console.log(`   Локальный: ${voice.localService}`);
      console.log(`   По умолчанию: ${voice.default}`);
      console.log(`   URI: ${voice.voiceURI || 'не указан'}`);
      console.log('---');
    });
    
    // Проверяем наличие Google TTS
    const googleVoices = voices.filter(v => v.name.toLowerCase().includes('google'));
    console.log(`Google TTS голосов: ${googleVoices.length}`);
    
    if (googleVoices.length === 0) {
      console.warn("⚠️ Google TTS не обнаружен. Возможно, нужно:");
      console.warn("1. Обновить Google TTS в Play Store");
      console.warn("2. Настроить языки в Настройки > Язык и ввод > Преобразование текста в речь");
      console.warn("3. Выбрать Google TTS как основной движок");
    }
  }
}

// Дополнительная функция для принудительной загрузки голосов на Android
function forceLoadAndroidVoices() {
  if (/Android/.test(navigator.userAgent)) {
    console.log("Принудительная загрузка голосов Android...");
    ...
    for (let i = 0; i < 3; i++) {
      setTimeout(() => {
        ...
        // Попытка получить голоса после каждой активации
        setTimeout(() => {
          const voices = window.speechSynthesis.getVoices();
          console.log(`Попытка ${i + 1}: найдено ${voices.length} голосов`);
        }, 500);
      }, i * 1000);
    }
...
// ВАЖНО ДЛЯ iOS: Добавьте эти функции для инициализации

// 1. Инициализация при первом взаимодействии пользователя
...
// 2. Вызовите это при первом клике/тапе пользователя
...
// 3. Дополнительная инициализация при загрузке страницы
document.addEventListener('DOMContentLoaded', () => {
  const isAndroid = /Android/.test(navigator.userAgent);
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
  
  if (isAndroid) {
    console.log("Android обнаружена. Начинаем инициализацию голосов...");
    forceLoadAndroidVoices().then(() => {
      console.log('Android голоса инициализированы');
    });
  } else if (isIOS) {
