  ...
  if ('speechSynthesis' in window) {
    ...
    const onVoicesChanged = () => {
      ...
      const findFallbackVoice = (isFemale = false) => {
        // Сначала ищем UK английские голоса
        let ukVoices = voices.filter(voice => 
          voice.lang === 'en-GB' || voice.lang === 'en-UK'
        );
        
        // Если UK голосов нет, ищем любые английские
        if (ukVoices.length === 0) {
          ukVoices = voices.filter(voice => 
      ...
      utterance.rate = 1;
      utterance.pitch = version === 'girl' ? 1.2 : 1.0; // Немного выше тон для женского голоса
...
function preloadVoices() {
  if ('speechSynthesis' in window) {
    ...
    // На некоторых устройствах нужно создать пустое высказывание
