...
// exercise 2-------------------------------------------------------------------------
// Common speech function for both exercises
...
function speakText(text) {
  if (!('speechSynthesis' in window) || isMobileDevice()) {
    fallbackToMP3(text);
    return;
  }
  ...
  const onVoicesChanged = () => {
    ...
    if (browser === "chrome") {
      ...
      maleVoice = voices.find(voice => voice.name.includes("Microsoft") && voice.lang === "en-GB");
      femaleVoice = voices.find(voice => voice.name.includes("Microsoft") && voice.lang === "en-GB");
...
function fallbackToMP3(text) {
  const safeText = text.trim(); // Без нижних подчёркиваний, пробелы сохраняем
  const audio = new Audio(`src/audio_fallback/${safeText}.mp3`);
  audio.play().catch(e => console.warn("MP3 проигрывание не удалось:", e));
}
...
function nextQuestion3() {
  imageCurrentIndex++; // Увеличиваем индекс текущего вопроса
  loadNextQuestion3(); // Загружаем следующий вопрос
}
