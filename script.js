...
function preloadVoices() {
  if ('speechSynthesis' in window) {
    ...
    if (isIOS) {
      ...
      dummy.volume = 0;
      dummy.rate = 10; // Быстро проговорим
...
// Функция для отладки голосов на iOS
function debugiOSVoices() {
  if (/iPad|iPhone|iPod/.test(navigator.userAgent)) {
    const voices = window.speechSynthesis.getVoices();
    console.log("=== Отладка голосов iOS ===");
    voices.forEach((voice, index) => {
      console.log(`${index}: "${voice.name}" | Lang: ${voice.lang} | Default: ${voice.default} | Local: ${voice.localService}`);
    });
    
    // Тестируем каждый голос
    voices.forEach((voice, index) => {
      if (index < 5) { // Тестируем только первые 5 голосов
        setTimeout(() => {
          console.log(`Тестируем голос: ${voice.name}`);
          const test = new SpeechSynthesisUtterance(`Test ${index + 1}`);
          test.voice = voice;
          test.volume = 0.3;
          test.rate = 1.5;
...
// Для тестирования доступных голосов
function testVoices() {
  const voices = window.speechSynthesis.getVoices();
